...
//   if (token == NULL) {
//     printf("radix:1 \n");
//     if (!(isupper(id_str[0])) || !(isdigit(id_str[1])))
//       return -2;
//     for (int l = 0; l < NUM_LETTERS; l++) {
//       if (id_str[0] == numeral_l[l]) {
//         printf("id_str[0] %c", id_str[l]);
//         letter = l;
//         break;
//       }
//     }
//     for (int d = 0; d < NUM_DIGITS; d++) {
//       if (id_str[1] == numeral_d[d]) {
//         printf("id_str[1] %c", id_str[d]);
//         digit = d;
//         break;
//       }
//     }
    ...
    while (token != NULL) {
      ...
      for (int l = 0; l < NUM_LETTERS; l++) {
        ...
        if (num_chr[radix][0] == numeral_l[l]) {
          printf("1:%c ", num_chr[radix][0]);
          letter = l;
        }
      }
      for (int d = 0; d < NUM_DIGITS; d++) {
        if (num_chr[radix][1] == numeral_d[d]) {
          printf("2:%c", num_chr[radix][1]);
          digit = d;
        }
      }
      if ((letter<0)||(digit<0)) return -2;
      numeral[radix] = letter * NUM_DIGITS + digit;
      printf("Numeral: %d\n",numeral[radix]);
      radix++; 
      if(radix>MAX_POS) return -2;
      token = strtok(NULL, "-");
    }
...
__uint128_t numeral_to_uint(uint8_t *numeral,int8_t radix) {
 ...
 __uint128_t res = 0;
  for (int i = 1; i <= radix; i++) {
     __uint128_t base = 1;
    for(int j=0;j<radix-i;j++){  base *= BASE;}
    res0 = base * numeral[i-1];
    printf("I:%d " ,i);
    printf("BASER: %llu ",base);
    printf("NUMERAL:%d ",numeral[i-1]);
    printf("RES: %llu \n",res0);
    res+=res0 ;
  }
  printf("RES: %llu \n",res);
